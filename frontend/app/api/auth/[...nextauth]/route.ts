...
export const authOptions: NextAuthOptions = {
  ...
  callbacks: {
    ...
    async signIn({ user, account }) {
      // Only allow google/github in your app
      ...
      try {
        ...
        const providerKey =
          account.provider === "google" ? "googleId" : account.provider === "github" ? "githubId" : null

        if (existingUser) {
          const updateData: any = {}
          if (providerKey && !existingUser[providerKey]) updateData[providerKey] = account.providerAccountId
          if (!existingUser.username && user.name) updateData.username = user.name
          ...
          if (providerKey) newUser[providerKey] = account.providerAccountId
